<?php
	...
	if(isset($key) && isset($login)) {
		...
		if($check==$key) {
			...
			header("Location: zapomnialem_hasla.php");
			exit();	
	...
	if(isset($_POST['vhaslo'])) $vhaslo =  trim($_POST['vhaslo']);
	...
	if(isset($haslo) && isset($vhaslo) && isset($login)) {
		...
		$tekst = 'Witaj '.$login.', aby wygenerować nowe hasło przejdź w podany adres:\n http://starozytnosc-grecja.hol.es/users/newpass.php?key='.$token.'&login='.$login.'';
